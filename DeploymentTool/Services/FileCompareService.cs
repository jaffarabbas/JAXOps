...
public class FileCompareService
{
    ...
    public async Task<List<FileChange>> ScanSingleFolderAsync(string folder, DateTime sinceDate)
    {
        return await Task.Run(() =>
        {
            var changes = new List<FileChange>();
            ...
            foreach (var file in Directory.EnumerateFiles(folder, "*", SearchOption.AllDirectories))
            {
                ...
                if (info.LastWriteTime < sinceDate)
                    continue;

                var relativePath = Path.GetRelativePath(folder, file);
                var status = info.CreationTime >= sinceDate ? ChangeStatus.New : ChangeStatus.Modified;
